public class TimeMap {
    ...
    public string Get(string key, int timestamp) {
        ...
        foreach (int entry in keyStore[key].Keys) {
            // look thru the all the timestamps, if any entry is less than or equal to the target, we want to capture that
            // if it equals to the target then that's requirement #3
            // we also get the last value before the target, that's #2
